...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.error('Get account ban status error:', error)
    return errorResponse('Internal server error', 500, request.headers.get('origin'))
  }
}
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!user.banned) {
      return errorResponse('Account is not banned', 400, request.headers.get('origin'))
    }

    const isValid = await bcrypt.compare(validated.password, user.passwordHash)
    if (!isValid) {
      logger.warn('Account unban failed: Invalid password', auth.userId)
      return errorResponse('Invalid password', 401, request.headers.get('origin'))
    }
    ...
    if (user.twoFactorEnabled) {
      ...
      if (!isValid2FA) {
        logger.warn('Account unban failed: Invalid 2FA code', auth.userId)
        return errorResponse('Invalid 2FA verification code', 401, request.headers.get('origin'))
      }
    }
    ...
    logger.info(`User account unbanned: ${user.email}`, auth.userId)

    return jsonResponse(
      { success: true, message: 'Account unbanned successfully' },
      200,
      request.headers.get('origin')
    )
  } catch (error: any) {
    if (error.name === 'ZodError') {
      logger.warn('Unban account validation error:', error.errors)
      return errorResponse(error.errors[0].message, 400, request.headers.get('origin'))
    }
    logger.error('Unban account error:', error)
    return errorResponse('Internal server error', 500, request.headers.get('origin'))
  }
}